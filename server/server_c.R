...
output$Total_Tornadoes_Per_Hour =  renderDataTable({    if(input$show_tables)  {     formatStyle(datatable(
...
output$Total_Tornadoes_Given_Distance =  renderDataTable({    if(input$show_tables)  {     formatStyle(datatable(
...
output$Injuries_Fatalities_Loss_Year =  renderDataTable({    if(input$show_tables)  {     formatStyle(datatable(
...
output$Injuries_Fatalities_Loss_Month =  renderDataTable({    if(input$show_tables)  {     formatStyle(datatable(
...
output$Injuries_Fatalities_Loss_Hour =  renderDataTable({    if(input$show_tables)  {     formatStyle(datatable(
...
output$Counties_Most_Hit_by_Tornadoes =  renderDataTable({    if(input$show_tables)  {     formatStyle(datatable(
...
output$Total_Tornadoes_Per_Hour_Barplot = renderPlotly({
  ...
  switch( time_format_type,
          ...
          "24_Hour" = plot_ly( Total_Tornadoes_Per_Hour_Plot_data, x = ~Format_24_Hour, y = ~each_hour_total, size = ~each_hour_total, name = 'Each Hour Total', type = 'bar', marker = list(color = 'rgb(205, 12, 24)')) %>%
